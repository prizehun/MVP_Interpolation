...
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=50, dist_thresh=3000, num_camera=6, intrinsics=None, transforms=None):
    ...
    #points = 2d projected point, raw_points = real lidar points
    ...
    mask = nearest_dist < dist_thresh # & (nearest_dist4 - nearest_dist < 2)
    
    #print(nearest_dist)
    indices = valid_mask.nonzero(as_tuple=False).reshape(-1)

    #nearest_dist2=nearest_dist2.tolist()
    bool_array1=nearest_dist < dist_thresh
    bool_array2=nearest_dist2 < dist_thresh
    bool_array3=nearest_dist3 < dist_thresh

    bool_array1=bool_array1.tolist()
    bool_array2=bool_array2.tolist()
    bool_array3=bool_array3.tolist()

    nearest_indices=nearest_indices.tolist()
    nearest_indices2=nearest_indices2.tolist()
    nearest_indices3=nearest_indices3.tolist()

    for k in range(len(bool_array1)) :
        if (bool_array1[k]==True and bool_array2[k]==False) :
            nearest_indices2[k]=nearest_indices[k]
        if (bool_array1[k]==True and bool_array3[k]==False) :
            nearest_indices3[k]=nearest_indices[k]

    nearest_indices=torch.tensor(nearest_indices, dtype=torch.long, device='cuda:0')
    nearest_indices2=torch.tensor(nearest_indices2, dtype=torch.long, device='cuda:0')
    nearest_indices3=torch.tensor(nearest_indices3, dtype=torch.long, device='cuda:0')    
    ...
    for i in range(num_camera):
        ...
        for j in range(len(x1[0])):
            ...
            if (y2[0][j] > yv[j]) != (y3[0][j] > yv[j]):
                ...
                if (xv[j] < atX) :
                    cross += 1                
            if (y3[0][j] > yv[j]) != (y1[0][j] > yv[j]):
                atX = (x1[0][j]-x3[0][j])*(yv[j]-y3[0][j])/(y1[0][j]-y3[0][j])+x3[0][j]
                if (xv[j] < atX) :
                    cross += 1                                                             
            if cross%2 > 0 :
                #dist1=math.sqrt((x1[0][j]-xv[j])**2 + (y1[0][j]-yv[j])**2)
                #dist2=math.sqrt((x2[0][j]-xv[j])**2 + (y2[0][j]-yv[j])**2)
                #dist3=math.sqrt((x3[0][j]-xv[j])**2 + (y3[0][j]-yv[j])**2)
                xv[j]=(x1[0][j]+x2[0][j]+x3[0][j])/3+0.00001*j
                yv[j]=(y1[0][j]+y2[0][j]+y3[0][j])/3+0.00001*j
                zv=(z1[0][j]+z2[0][j]+z3[0][j])/3
                #print(depth)
                #print(z1[0][j])
                z.append(zv)
            if cross%2 == 0 :
        ...
        del x1; del x2; del x3; del y1; del y2; del y3 #memory release
        del z1; del z2; del z3
        del per_camera_indices2; del per_camera_indices3
...
def main(args):
    ...
    data_loader = DataLoader(
        ...
        batch_size=1,
        num_workers=5, #cpu core allocate
