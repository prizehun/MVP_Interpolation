...
def add_virtual_mask(masks, labels, points, raw_points, num_virtual=50, dist_thresh=3000, num_camera=6, intrinsics=None, transforms=None):
    ...
    mask = nearest_dist2 < dist_thresh # & (nearest_dist4 - nearest_dist < 2)
        ...
        for j in range(len(x1[0])):
            ...
            if (y3[0][j] > yv[j]) != (y1[0][j] > yv[j]):
                ...
                if (xv[j] < atX) :
                    cross += 1                                                             
            if cross%2 > 0 :
                a=y1[0][j]*(z2[0][j]-z3[0][j])+y2[0][j]*(z3[0][j]-z1[0][j])+y3[0][j]*(z1[0][j]-z2[0][j])
                b=z1[0][j]*(x2[0][j]-x3[0][j])+z2[0][j]*(x3[0][j]-x1[0][j])+z3[0][j]*(x1[0][j]-x2[0][j])
                c=x1[0][j]*(y2[0][j]-y3[0][j])+x2[0][j]*(y3[0][j]-y1[0][j])+x3[0][j]*(y1[0][j]-y2[0][j])
                d=-x1[0][j]*(y2[0][j]*z3[0][j]-y3[0][j]*z2[0][j])-x2[0][j]*(y3[0][j]*z1[0][j]-y1[0][j]*z3[0][j])-x3[0][j]*(y1[0][j]*z2[0][j]-y2[0][j]*z1[0][j])
                depth=-(a*xv[j]+b*yv[j]+d)/c
                #print(depth)
                #print(z1[0][j])
                if abs(depth-z1[0][j]) > 0.5:
                    if abs((z1[0][j]+z2[0][j])/2 -z1[0][j]) > 0.5:
                        z.append(z1[0][j])
                    else :
